(function () {
  ...
  if (!room) {
    document.getElementById('startOverlay').innerHTML = '<div class="overlay-inner"><h2>افتح الشاشة من رابط اللعبة</h2><p class="muted">اطلب رابط شاشة العرض من المضيف (يحتوي رمز الغرفة).</p><a href="/" class="btn btn-accent" style="margin-top:12px;">الرئيسية</a></div>';
    return;
  }

  var socket = io({ auth: { role: 'display', room: room } });
  ...
  socket.on('tick', function (t) { if (lastState) { lastState.timeLeft = t.timeLeft; updateHeader(lastState); } });
  ...
  socket.on('auth:error', function (e) {
    ...
    document.getElementById('startOverlay').innerHTML = '<div class="overlay-inner"><h2>' + (e.error || 'تعذّر الاتصال') + '</h2><a href="/" class="btn btn-accent" style="margin-top:12px;">الرئيسية</a></div>';
  });

  function show(id, on) { document.getElementById(id).classList[on ? 'remove' : 'add']('hidden'); }

  function updateHeader(s) {
    document.getElementById('compName').textContent = s.name || 'لعبة الأفخاخ';
    ...
    var dur = (s.question && s.question.timeLimitSec) || 45;
    if (s.questionState === 'lies' || s.questionState === 'pick') {
      ...
      if (s.timeLeft <= 5 && s.timeLeft > 0) { Sound.countdown(); ringTime.style.color = '#ffd0d4'; } else ringTime.style.color = '';
    } else {
  ...
  function render(s) {
    if (!s || !s.active) { lastState = s; show('lobby', true); show('stage', false); return; }
    lastState = s;
    updateHeader(s);
  ...
  function renderLobby(s) {
    document.getElementById('lobbyRoom').textContent = room;
    ...
    fetch('/api/config').then(function (r) { return r.json(); }).then(function (c) {
      document.getElementById('joinUrl').textContent = (c.baseUrl || location.origin) + '/player?room=' + room;
    }).catch(function () {});
  ...
  function renderRevealBoard(rows) {
    ...
    rows.slice(0, 5).forEach(function (g) {
      rb.appendChild(el('div', { class: 'rb-item' + (g.rank === 1 ? ' r1' : '') }, [
        el('span', { class: 'rb-rank' }, ['#' + g.rank]), el('span', {}, [g.name]), el('span', { class: 'rb-score mono' }, [fmt(g.score)])
      ]));
    });
  }
  ...
  function launchConfetti() {
    ...
    for (var i = 0; i < 90; i++) {
      ...
      p.style.animationDuration = (2.6 + Math.random() * 2.4) + 's'; p.style.animationDelay = (Math.random() * 1.2) + 's';
      p.style.height = (10 + Math.random() * 10) + 'px'; p.style.transform = 'rotate(' + Math.random() * 360 + 'deg)';
      wrap.appendChild(p);
    }
  ...
  function buildPodium(standings) {
    var podium = document.getElementById('podium'); var rest = document.getElementById('podiumRest');
    podium.innerHTML = '';
